...
Deep Agents use the filesystem as the interface for memory. The agent reads and writes files, and the [backend](/oss/javascript/deepagents/backends) you configure determines how long that information persists.
...
| [**Information type**](#information-type) | [Semantic](/oss/javascript/concepts/memory#semantic-memory) (facts), [episodic](/oss/javascript/concepts/memory#episodic-memory) (experiences), or [procedural](/oss/javascript/concepts/memory#procedural-memory) (instructions) | What kind of information is it? |
...
|---|---|---|
| [Semantic](/oss/javascript/concepts/memory#semantic-memory) | Facts and knowledge | [User preferences](#user-preferences), [knowledge base](#knowledge-base) |
| [Episodic](/oss/javascript/concepts/memory#episodic-memory) | Past experiences | [Episodic log](#episodic-log) |
| [Procedural](/oss/javascript/concepts/memory#procedural-memory) | Instructions and rules | [agents.md](#the-agentsmd-pattern), [persistent skills](#persistent-skills) |

Semantic memories can be stored as a single continuously updated [profile](/oss/javascript/concepts/memory#profile) or as a [collection](/oss/javascript/concepts/memory#collection) of individual documents, each with different tradeoffs. For a deeper conceptual overview, see the [memory concepts guide](/oss/javascript/concepts/memory).
...
As conversations grow, the full message history may not fit in the LLM's context window, or may degrade response quality. Deep Agents handle this automatically through [context engineering](/oss/javascript/deepagents/context-engineering), but you can also manage short-term memory manually. For techniques like summarization, trimming, and filtering, see the [manage short-term memory](/oss/javascript/langgraph/add-memory#manage-short-term-memory) guide.
...
To enable long-term memory, use a [`CompositeBackend`](https://reference.langchain.com/javascript/deepagents/backends/CompositeBackend) that routes `/memories/` to persistent storage. The recommended approach is a [`StoreBackend`](https://reference.langchain.com/javascript/deepagents/backends/StoreBackend) backed by a LangGraph [Store](/langsmith/custom-store), but because backends are pluggable, you can persist memories to any external data source (a vector database, a custom API, cloud storage) by implementing the [backend protocol](/oss/javascript/deepagents/backends#protocol-reference).
...
<Note>
`CompositeBackend` strips the route prefix before storing. `/memories/agents.md` is stored as `/agents.md` in the `StoreBackend`. The agent always uses the full path. See [CompositeBackend](/oss/javascript/deepagents/backends#compositebackend-router) for details.
</Note>
...
- For most agents, write memories [in the hot path](/oss/javascript/concepts/memory#in-the-hot-path) (during the conversation). It's the simplest approach.
