...
| [Procedural](/oss/javascript/concepts/memory#procedural-memory) | Instructions and rules | Self-improving system prompts, [persistent skills](#persistent-skills) |
...
Then use [middleware](/oss/javascript/langchain/middleware/custom) or your system prompt to load this data into the agent's context at the start of each conversation. The agent can read and follow the policies but has no mechanism to alter them.
...
[Skills](/oss/javascript/deepagents/skills) are reusable agent capabilities defined as `SKILL.md` files. When stored in `/memories/`, they become a form of [procedural memory](/oss/javascript/concepts/memory#procedural-memory) — the agent can create, refine, and reuse its own workflows across conversations.
