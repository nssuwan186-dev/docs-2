...
|---|---|---|
| **Duration** | [Short-term](/oss/python/deepagents/context-engineering) (single conversation) or [long-term](#quick-start) (across conversations) | How long does it last? |
| [**Information type**](#memory-patterns) | [Semantic](/oss/python/concepts/memory#semantic-memory) (facts), [episodic](/oss/python/concepts/memory#episodic-memory) (experiences), or [procedural](/oss/python/concepts/memory#procedural-memory) (instructions) | What kind of information is it? |
...
<Note>
`CompositeBackend` strips the route prefix before storing. A file written to `/memories/preferences.md` by an agent with namespace `(user_id,)` is stored at key `/preferences.md` under the `("user-123",)` namespace in the `StoreBackend`. The agent always uses the full path. See [CompositeBackend](/oss/python/deepagents/backends#compositebackend-router) for details.
</Note>
...
- **[Procedural](/oss/python/concepts/memory#procedural-memory)** — instructions and rules that shape how the agent behaves (agents.md, user preferences, skills)
- **[Semantic](/oss/python/concepts/memory#semantic-memory)** — facts and knowledge the agent can reference (knowledge bases, project context)
- **[Episodic](/oss/python/concepts/memory#episodic-memory)** — past experiences the agent can learn from (conversation history, debugging sessions)
...
For most applications, start with the hot path. Add background consolidation when you need to reduce latency or improve memory quality across many conversations. See the [memory concepts guide](/oss/python/concepts/memory#writing-memories) for more on these tradeoffs.
...
Load memories into the system prompt at conversation start using a `wrap_model_call` [middleware](/oss/python/langchain/middleware/custom). This guarantees the agent always has context, at the cost of tokens on every model call. The [Quick start](#quick-start) example demonstrates this approach.
...
[Skills](/oss/python/deepagents/skills) are a good example of progressive retrieval: only the frontmatter is loaded at startup, and the full skill content is read only when the agent determines it's relevant.
...
To bridge this gap, seed memories into the sandbox before the agent runs and sync updated memories back after it finishes. Use [custom middleware](/oss/python/langchain/middleware/custom) with `before_agent` and `after_agent` hooks to transfer files across the sandbox boundary:
...
- The agent runs in a [sandbox](/oss/python/deepagents/sandboxes) with an isolated filesystem
- Memories or [skills](/oss/python/deepagents/skills) need to be executable inside the container (e.g., skill scripts the agent runs via `execute`)
...
For implementation details and a full code example, see [file transfers](/oss/python/deepagents/going-to-production#file-transfers) and the [syncing skills and memories](/oss/python/deepagents/going-to-production#file-transfers) example in the going to production guide.
