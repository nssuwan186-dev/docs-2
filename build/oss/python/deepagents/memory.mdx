...
Memory lets your agent learn and improve across conversations. Deep Agents makes memory first class with a filesystem-based system: the agent reads and writes memory as files, and you control where those files are stored using [backends](/oss/python/deepagents/backends).
...
To persist memory across conversations, route `/memories/` to a persistent backend using a [`CompositeBackend`](https://reference.langchain.com/python/deepagents/backends/composite/CompositeBackend). How you scope the backend controls who can see the memory: [agent-scoped](#agent-scoped-memory) (shared across all users) or [user-scoped](#user-scoped-memory) (isolated per user).
...
In this pattern:
- `AGENTS.md` and [skills](/oss/python/deepagents/skills) live in the filesystem and are editable by the agent
...
- [Skills](/oss/python/deepagents/skills) are not editable by the agent
...
`StoreBackend` works with any LangGraph [`BaseStore`](https://reference.langchain.com/python/langchain-core/stores/BaseStore) implementation. The store is LangGraph's cross-thread persistence layer: it lets you save and retrieve data scoped to any custom namespace, not just a single thread. When deploying to [LangSmith Deployment](/langsmith/deployment), the platform provisions a store automatically.

For local development, [`InMemoryStore`](https://reference.langchain.com/python/langchain-core/stores/InMemoryStore) is sufficient. It stores data in an in-memory dictionary and supports basic operations (`put`, `search`, `get`) as well as [semantic search](/oss/python/langgraph/persistence#semantic-search) with embeddings.

For production, use a persistent store:

| Store | Use case |
|---|---|
| [`PostgresStore`](https://reference.langchain.com/python/langgraph/store/#langgraph.store.postgres.PostgresStore) | Recommended for most production deployments. Supports async via `AsyncPostgresStore`. |
| @[`RedisStore`] | Low-latency access patterns. Supports async via `AsyncRedisStore`. |
| Custom | Implement the [`BaseStore`](https://reference.langchain.com/python/langchain-core/stores/BaseStore) interface to back memory with a vector database, custom API, or cloud storage. See the [backend protocol](/oss/python/deepagents/backends#protocol-reference). |
...
For more on store operations, semantic search, and using stores inside LangGraph nodes, see the [persistence guide](/oss/python/langgraph/persistence#memory-store).
...
| **Retrieval** | How are memories read? | Loaded into prompt (default), on demand (e.g., [episodic memory](#episodic-memory), [skills](/oss/python/deepagents/skills)), or via [custom middleware](#custom-memory-loading) |
...
To enforce read-only memory, use [policy hooks](/oss/python/deepagents/backends#add-policy-hooks) on the backend that reject write operations to specific paths:
...
By default, `MemoryMiddleware` loads memory files into the system prompt at the start of each conversation. For more control over how memories are injected, use [custom middleware](/oss/python/langchain/middleware/custom) to load memories yourself.
